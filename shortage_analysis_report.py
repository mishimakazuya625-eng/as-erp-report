...
def get_filter_options():
    """Fetch unique values for filters from DB"""
    conn = get_db_connection()
    try:
        customers = pd.read_sql_query("SELECT DISTINCT CUSTOMER FROM Product_Master ORDER BY CUSTOMER", conn)
        sites = pd.read_sql_query("SELECT DISTINCT SITE_CODE FROM Plant_Site_Master ORDER BY SITE_CODE", conn)
    finally:
        conn.close()
...
def load_data(target_customers=None, target_sites=None, target_statuses=None):
    ...
    # 1. Load Orders (Filtered by Status)
    if not target_statuses:
        target_statuses = ['OPEN', 'URGENT']
    
    # Postgres IN clause with tuple
    query_orders = """
        SELECT ORDER_KEY, PN, ORDER_QTY, DELIVERED_QTY, ORDER_STATUS, URGENT_FLAG 
        FROM AS_Order 
        WHERE ORDER_STATUS IN %s
    """
    orders = pd.read_sql_query(query_orders, conn, params=(tuple(target_statuses),))
    
    # 2. Load Product Master (Filtered by Customer and Site)
    query_products = "SELECT PN, PLANT_SITE, CUSTOMER FROM Product_Master WHERE 1=1"
    params_products = []
    
    if target_customers:
        query_products += " AND CUSTOMER IN %s"
        params_products.append(tuple(target_customers))
        
    if target_sites:
        query_products += " AND PLANT_SITE IN %s"
        params_products.append(tuple(target_sites))
        
    if params_products:
        products = pd.read_sql_query(query_products, conn, params=params_products)
    else:
        products = pd.read_sql_query(query_products, conn)
    
    # 3. Load BOM Master
    bom = pd.read_sql_query("SELECT PARENT_PN, CHILD_PKID, BOM_QTY FROM BOM_Master", conn)
    ...
    if latest_date:
        inventory = pd.read_sql_query("""
            SELECT PKID, PLANT_SITE, PKID_QTY 
            FROM Inventory_Master 
            WHERE SNAPSHOT_DATE = %s
        """, conn, params=(latest_date,))
    else:
        inventory = pd.DataFrame(columns=['PKID', 'PLANT_SITE', 'PKID_QTY'])
        
    # 5. Load Substitute Master
    substitutes = pd.read_sql_query("SELECT CHILD_PKID, SUBSTITUTE_PKID, DESCRIPTION FROM Substitute_Master", conn)
    ...
    return orders, products, bom, inventory, substitutes, latest_date, all_plant_sites
